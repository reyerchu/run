      ...
      <div className="container-narrow max-w-4xl">
        ...
        <div className="flex flex-wrap items-start gap-4 mb-8">
          <div className="flex-1 min-w-0">
            ...
            {event.nameEn && <p className="mt-1 text-lg text-gray-400">{event.nameEn}</p>}
          </div>
          <span className={`badge text-white text-sm ${color}`}>{label}</span>
        </div>
        ...
        <div className="rounded-xl border border-gray-100 bg-gray-50 p-6 mb-8">
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <div className="text-sm text-gray-400 mb-1">比賽日期</div>
              <div className="font-semibold">
                {isExpected
                  ? formatEstimatedDate(event.estimatedMonth ?? null, event.date)
                  : formatDate(event.date)}
              </div>
              {!isExpected && daysToRace !== null && daysToRace > 0 && <div className="text-sm text-emerald-600">{daysToRace} 天後</div>}
            </div>
            <div>
              <div className="text-sm text-gray-400 mb-1">地點</div>
              <div className="font-semibold">{event.city}{event.country ? `, ${event.country}` : ""}</div>
            </div>
            {isExpected ? (
              <div>
                <div className="text-sm text-gray-400 mb-1">報名時間</div>
              ...
              <>
                {event.registrationStart && (
                  <div>
                    <div className="text-sm text-gray-400 mb-1">報名開始</div>
                    <div className="font-semibold">{formatDate(event.registrationStart)}</div>
                  </div>
                )}
                {event.registrationEnd && (
                  <div>
                    <div className="text-sm text-gray-400 mb-1">報名截止</div>
                    <div className="font-semibold">{formatDate(event.registrationEnd)}</div>
                    {daysUntil(event.registrationEnd) > 0 && (
                      <div className="text-sm text-orange-500">剩餘 {daysUntil(event.registrationEnd)} 天</div>
                    )}
                  </div>
                )}
              </>
          ...
          {/* Distances */}
          <div className="mt-6">
            <div className="text-sm text-gray-400 mb-2">比賽距離</div>
            <div className="flex flex-wrap gap-2">
              {distances.map((d) => (
                <span key={d} className="badge bg-white border border-gray-200 text-gray-700">{d}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        {/* Status banners */}
        {isCompleted && (
          <div className="rounded-lg bg-gray-100 border border-gray-200 p-4 mb-8">
            <p className="text-sm text-gray-600">
              🏁 此賽事已於 {formatDate(event.date)} 完賽。以下資訊僅供參考。
            </p>
          </div>
        )}
        {isExpected && (
          <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 mb-8">
            <p className="text-sm text-amber-700">
              📅 此為週期性賽事，依據往年慣例推估舉辦時間。實際日期、報名方式及費用以主辦單位正式公告為準。
            </p>
          </div>
        )}
        ...
        {event.description && (
          <div className="mb-8">
            <h2 className="text-lg font-semibold mb-3">賽事簡介</h2>
        ...
        <div className="flex flex-wrap gap-3 mb-12">
          ...
          {event.registrationUrl && !isCompleted && computedStatus !== "closed" && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-full bg-emerald-500 px-6 py-2.5 text-sm font-semibold text-white hover:bg-emerald-400 transition-colors"
            >
              ✏️ 前往報名
            </a>
          )}
        ...
        {/* Reviews */}
        <div className="mb-12">
          <h2 className="text-lg font-semibold mb-4">💬 跑者心得 ({event.reviews.length})</h2>
          {event.reviews.length === 0 ? (
            <p className="text-gray-400 text-sm">尚無心得分享，成為第一個分享的跑者吧！</p>
          ) : (
        ...
        {/* Photos */}
        <div>
          <h2 className="text-lg font-semibold mb-4">📸 照片牆 ({event.photos.length})</h2>
          {event.photos.length === 0 ? (
            <p className="text-gray-400 text-sm">尚無照片，快來上傳你的比賽照片！</p>
          ) : (
        ...
        <div className="mt-8 text-center text-sm text-gray-400">
          ❤️ {event._count.favorites} 人收藏此賽事
        </div>
      </div>
    </section>
  );
